...
- [ ] **Image acquisition** (in progress): every spike so far leaned on a layer materialized by a one-time `docker pull`, and borrowing Docker's Administrators-ACLed store is not a shipping design. `pull` + `import` now **implement** acquisition from an anonymous registry straight into a store AspireHcs owns, via the OCI-tar path (base layers cannot travel through `ExportLayer` at all) — implemented and partly proven, not yet proven end to end. Measured so far, unelevated: pulling is digest-verified end to end, and extracting all 10 288 entries of `nanoserver:ltsc2025` takes 5.2 s with **no privileges whatsoever** — the gate turns out to be `ProcessBaseImage` at *finalize* time (`0x80070522`), not extraction. **Still unproven: the elevated import, and booting from our own store.** Details and the pending list: [docs/image-acquisition.md](docs/image-acquisition.md).
- [ ] **Unverified**: the documented claim that Hyper-V isolation lifts the host/image build-match constraint. No build-mismatched image has been booted on the reference host — treat it as untested, not true. The fixture now exists (`nanoserver:ltsc2022`, build 20348, pulled onto this 26200 host); the answer does not.
