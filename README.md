...
- [ ] **Privilege model** ([#33](https://github.com/joshmakestuff/AspireHcs/issues/33)): both spikes ran fully elevated. "Run your AppHost as admin" is not an acceptable developer story, and the one unelevated result we have is confounded by Docker's store ACL. This is the blocker on a container resource being designable.
- [ ] **Image acquisition**: both spikes leaned on a layer materialized by a one-time `docker pull`. Pulling and materializing layers without Docker is the bulk of the remaining effort.
