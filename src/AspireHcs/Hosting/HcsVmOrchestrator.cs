    ...
    ILogger logger)
{
    ...
    /// the convention from #62: once the guest is up, the values land in <c>/etc/aspire.env</c>,
    /// and a workload reads the file when it starts. The stated caveat travels with it — a
    /// workload that autostarts at boot may run before the file lands; one that reads the file
    /// when it starts is correct today. Tighter ordering is future work, not a blocker (#62).
    /// </para>
    /// <para>
    ...
    {
        IReadOnlyDictionary<string, string> environment = await GuestEnvironment.ResolveAsync(
        ...
        environment = await GuestReferences.RedirectLoopbackAsync(
            resource.Name,
