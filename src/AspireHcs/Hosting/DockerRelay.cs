...
/// The chain is #59's, measured end to end from inside a VM (HTTP 200 from a loopback-bound
/// Kestrel): guest → HNS gateway → docker-published port → socat → host.docker.internal → target.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// One multiplexing container, per #62's decision, mirroring Aspire's own tunnel shape (one
/// <c>aspire</c> container carrying all endpoints). Docker cannot add published ports to a
/// running container, so a target that arrives after the relay exists recreates it with the
/// superset — keeping every already-issued relay port on its number, so values injected into
/// earlier consumers stay valid. The recreate drops live relayed connections for the moment it
/// takes; consumers created after it see nothing.
/// </para>
/// <para>
/// The container is named with this process's id and scavenged by the same discipline as HCS
/// containers: an id this integration wrote, whose recorded pid is not running, and nothing else.
/// </para>
/// </remarks>
internal sealed class DockerRelay(IHostApplicationLifetime lifetime, ILogger<DockerRelay> logger)
{
    /// <summary>
    /// Identifies relay containers this integration owns; the pid suffix makes ownership
    ...
    /// <summary>
    /// The relay image. <c>socat</c> is the whole requirement, and this is the image the #59
    /// measurement ran.
    /// </summary>
    ...
    /// <summary>Target host port → the relay's published host port. Mutated only under the gate.</summary>
    private readonly Dictionary<int, int> _published = [];

    private bool _initialized;

    internal string ContainerName { get; } =
        $"{NamePrefix}{Environment.ProcessId.ToString(CultureInfo.InvariantCulture)}";
    ...
    /// picking a free port and claiming it. It is read back with <c>docker port</c> after create.
    /// </remarks>
    public async Task<int> EnsurePublishedAsync(int targetPort, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await RunDockerAsync(["rm", "-f", ContainerName], cancellationToken, ignoreFailure: true)
                .ConfigureAwait(false);

            List<string> arguments = ["run", "-d", "--name", ContainerName, "--entrypoint", "/bin/sh"];
            foreach ((int target, int host) in _published)
            {
                // Ports already handed out keep their numbers across the recreate — they are
                // baked into environments consumers already received.
                arguments.Add("-p");
                arguments.Add(FormattableString.Invariant($"{host}:{target}"));
            }

            arguments.Add("-p");
            arguments.Add(targetPort.ToString(CultureInfo.InvariantCulture));
            arguments.Add(Image);
            arguments.Add("-c");
            arguments.Add(BuildScript([.. _published.Keys, targetPort]));

            await RunDockerAsync(arguments, cancellationToken).ConfigureAwait(false);

            string mapping = await RunDockerAsync(
                ["port", ContainerName, FormattableString.Invariant($"{targetPort}/tcp")], cancellationToken)
                .ConfigureAwait(false);
            int hostPort = ParsePublishedPort(mapping);

            _published[targetPort] = hostPort;
            logger.LogInformation(
                "Relay {ContainerName} publishes 0.0.0.0:{HostPort} -> host.docker.internal:{TargetPort} ({Count} forward(s) total).",
                ContainerName, hostPort, targetPort, _published.Count);
    ...
    private async Task InitializeAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            await RunDockerAsync(["version", "--format", "{{.Server.Version}}"], cancellationToken)
                .ConfigureAwait(false);
        }
        catch (InvalidOperationException ex)
        {
            throw new InvalidOperationException(
                ...
                "bound to the host's loopback, which an HCS guest cannot reach (measured, #58), so " +
                "AspireHcs relays them through a Docker container. Start Docker Desktop (or another " +
        ...
        lifetime.ApplicationStopping.Register(() =>
        {
            try
            {
                ...
                RunDockerAsync(["rm", "-f", ContainerName], timeout.Token).GetAwaiter().GetResult();
            }
            catch (Exception ex)
            {
    ...
    private async Task ScavengeAbandonedRelaysAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            string listing = await RunDockerAsync(
                ["ps", "-a", "--filter", $"name={NamePrefix}", "--format", "{{.Names}}"], cancellationToken)
                .ConfigureAwait(false);
            ...
            foreach (string name in SelectScavengeable(names, ContainerName, livePids.Contains))
            {
                ...
                try
                {
                    ...
                    await RunDockerAsync(["rm", "-f", name], cancellationToken).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
    ...
    /// or belonging to this run is left alone.
    /// </summary>
    ...
    /// did not write it. The whole suffix must be the pid — <c>aspirehcs-relay-123x</c> is
    /// somebody else's container, never a candidate.
    /// </summary>
    internal static int? OwnerProcessId(string? name)
    {
        ...
        return int.TryParse(name.AsSpan(NamePrefix.Length), NumberStyles.None, CultureInfo.InvariantCulture, out int pid)
            ? pid
            : null;
    }
    ...
    /// <c>wait</c> so the container lives as long as its forwarders do — the multiplexing shape
    /// #62 chose. Inside the container each listener uses the target's own port number; the
    /// numbers are distinct by construction (they key <see cref="_published"/>) and nothing else
    /// runs in the container to collide with.
    /// </summary>
    ...
    private static async Task<string> RunDockerAsync(
        IReadOnlyList<string> arguments, CancellationToken cancellationToken, bool ignoreFailure = false)
    {
        ...
        // as the child fills the pipe nothing is reading. Same trap as the hcsctl seam.
