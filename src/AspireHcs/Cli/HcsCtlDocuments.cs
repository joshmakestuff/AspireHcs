...
internal sealed record HcsCtlVmCreateDocument
{
    ...
    /// <summary>
    /// Data disks attached at SCSI LUN 1..n in order — differencing children under
    /// copy-on-write, like <see cref="DiskPath"/>. Empty when the VM has no extra disks.
    /// </summary>
    [JsonPropertyName("disks")]
    public IReadOnlyList<string> Disks { get => field ?? []; init; } = [];
    ...
    /// <summary>Generated by hcsctl, or the requested MAC echoed back normalized.</summary>
    [JsonPropertyName("macAddress")]
    public string? MacAddress { get; init; }

    /// <summary>Access VLAN of the NIC's switch port; 0 means untagged (hcsctl omits it then).</summary>
