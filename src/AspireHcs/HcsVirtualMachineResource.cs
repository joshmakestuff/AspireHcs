    ...
    : Resource(name), IResourceWithEndpoints, IResourceWithConnectionString, IResourceWithEnvironment
{
    ...
    /// <summary>
    /// Extra VHDXs attached at SCSI LUN 1..n, in <c>WithDisk</c> order. Full paths. Like the boot
    /// disk they are booted as differencing children; the bases are never written.
    /// </summary>
    internal List<string> DataDiskPaths { get; } = [];

    /// <summary>
    /// MAC requested via <c>WithMacAddress</c>, normalized <c>XX-XX-XX-XX-XX-XX</c>; null lets
    /// hcsctl generate one. For guests whose network config is pinned to a specific address.
    /// </summary>
    internal string? RequestedMacAddress { get; set; }
    ...
    /// the guest anything: no <c>vm ip</c>, no <c>/etc/aspire.env</c> delivery, no hvsocket
    /// forwards. Endpoints resolve here once the address answers TCP.
    /// </summary>
    internal string? GuestAddress { get; set; }

    /// <summary>How long the fixed address gets to answer before the boot fails. Appliances boot slowly.</summary>
    internal TimeSpan GuestAddressTimeout { get; set; } = TimeSpan.FromMinutes(15);

    /// <summary>True when <see cref="GuestAddress"/> declares the VM agentless.</summary>
    internal bool IsAgentless => GuestAddress is not null;
    ...
    /// <summary>
    /// The MAC and HCN endpoint learned from the create result — generated by hcsctl, or the
    /// requested MAC echoed back normalized. Both are null until the VM has been created.
    /// hcsctl's store owns the MAC; it must survive a stop/start for the DHCP lease to hold.
    /// </summary>
